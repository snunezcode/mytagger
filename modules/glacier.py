...
def get_service_types(account_id, region, service, service_type):
    """
    AWS Glacier resources that support tagging.
    Based on: https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/glacier/client/add_tags_to_vault.html
    
    Glacier supports tagging for:
    - Vault (glacier vaults for long-term archival storage)
    """
    ...
    resource_configs = {
        'Vault': {
            ...
            'key': 'VaultList',
            'id_field': 'VaultName',
            'name_field': 'VaultName',
            'date_field': 'CreationDate',
            'nested': False,
            'arn_format': 'arn:aws:glacier:{region}:{account_id}:vaults/{resource_id}'
        }
    }
...
def discovery(self, session, account_id, region, service, service_type, logger):    
    ...
    try:
        ...
        # Glacier is regional
        client = session.client('glacier', region_name=region)
        
        if not hasattr(client, config['method']):
            raise ValueError(f"Method {config['method']} not available for glacier client")
        ...
        for page in page_iterator:
            ...
            for item in items:
                ...
                resource_tags = {}
                try:
                    tags_response = client.list_tags_for_vault(vaultName=resource_id)
...
def tagging(account_id, region, service, client, resources, tags_string, tags_action, logger):
    ...
    glacier_client = session.client('glacier', region_name=region)

    for resource in resources:            
        try:
            if tags_action == 1:
                # Add tags - Convert to Glacier format (dict)
                glacier_tags = {tag['Key']: tag['Value'] for tag in tags}
                glacier_client.add_tags_to_vault(
                    vaultName=resource.identifier,
                    Tags=glacier_tags
                )
