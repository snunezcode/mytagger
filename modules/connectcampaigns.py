...
def get_service_types(account_id, region, service, service_type):
    """
    Amazon Connect Campaign Service resources that support tagging.
    
    Based on AWS Connect Campaigns documentation:
    https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/connectcampaigns.html
    
    Connect Campaigns supports tagging for:
    - Campaign (Outbound campaigns for automated calling)
    """
...
def discovery(self, session, account_id, region, service, service_type, logger):    
    ...
    try:
        ...
        try:
            client = session.client('connectcampaigns', region_name=region, config=client_config)
        except Exception as e:
            logger.warning(f"Connect Campaigns client creation failed in region {region}: {str(e)}")
            return f'{service}:{service_type}', "success", "", []
        
        if not hasattr(client, config['method']):
            logger.warning(f"Method {config['method']} not available for connectcampaigns client")
            return f'{service}:{service_type}', "success", "", []

        method = getattr(client, config['method'])
        params = {}
        
        # Handle Connect Campaigns API calls with proper error handling and retry logic
        try:
            logger.info(f"Calling Connect Campaigns {config['method']} in region {region}")
            ...
            logger.warning(f"Connect Campaigns timeout in region {region}: {str(e)}")
            return f'{service}:{service_type}', "success", "", []
        except ClientError as e:
            error_code = e.response.get('Error', {}).get('Code', 'Unknown')
            if error_code in ['UnauthorizedOperation', 'AccessDenied', 'InvalidAction']:
                logger.warning(f"Connect Campaigns not available in region {region}: {error_code}")
                return f'{service}:{service_type}', "success", "", []
            elif error_code in ['ResourceNotFoundException', 'InvalidParameterException']:
                logger.info(f"Connect Campaigns {service_type} not found in region {region}")
                return f'{service}:{service_type}', "success", "", []
            else:
                logger.error(f"Connect Campaigns API error in region {region}: {str(e)}")
                return f'{service}:{service_type}', "error", str(e), []
        except Exception as e:
            logger.warning(f"Connect Campaigns general error in region {region}: {str(e)}")
            return f'{service}:{service_type}', "success", "", []
            ...
            for item in items:
                try:
                    ...
                    try:
                        ...
                        if tags_response:
                            # Connect Campaigns returns tags as a dictionary
                            resource_tags = tags_response.get('tags', {})
                        else:
                            logger.warning(f"Failed to get tags for Connect Campaigns resource {resource_name}")
                            resource_tags = {}
                            
                    except (ConnectTimeoutError, ReadTimeoutError):
                        logger.warning(f"Timeout retrieving tags for Connect Campaigns resource {resource_name}")
                        resource_tags = {}
                        ...
                        if tag_error_code in ['ResourceNotFoundException', 'AccessDenied']:
                            logger.info(f"No tags found for Connect Campaigns resource {resource_name}")
                            resource_tags = {}
                        else:
                    ...
                    if service_type == 'Campaign':
                        additional_metadata = {
                            ...
                            'dialerConfig': item.get('dialerConfig', {}),
                            'outboundCallConfig': item.get('outboundCallConfig', {}),
                            'tags': item.get('tags', {})
                        }
                    ...
                    logger.warning(f"Error processing Connect Campaigns item: {str(item_error)}")
                    continue
        ...
        status = "error"
        error_message = str(e)
        logger.error(f"Error in Connect Campaigns discover function: {error_message}")
...
def tagging(account_id, region, service, client, resources, tags_string, tags_action, logger):
    ...
    # Create Connect Campaigns client with timeout protection
    ...
    try:
        campaigns_client = session.client('connectcampaigns', region_name=region, config=client_config)
    except Exception as e:
        logger.error(f"Failed to create Connect Campaigns client: {str(e)}")
        return []
        ...
        try:
            def tag_resource():
                if tags_action == 1:  # Add tags
                    # Convert tags to dictionary format for Connect Campaigns
