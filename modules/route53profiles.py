...
def get_service_types(account_id, region, service, service_type):
    """
    Route53 Profiles resources that support tagging.
    Based on: https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/route53profiles/client/tag_resource.html
    """

    resource_configs = {
        'Profile': {
            'method': 'list_profiles',
            'key': 'ProfileSummaries',
            'id_field': 'Id',
            'name_field': 'Name',
            'date_field': 'CreationTime',
            'nested': False,
            'arn_format': 'arn:aws:route53profiles:{region}:{account_id}:profile/{resource_id}'
        },
        'ProfileAssociation': {
            'method': 'list_profile_associations',
            'key': 'ProfileAssociations',
            'id_field': 'Id',
            'name_field': 'Name',
            'date_field': 'CreationTime',
            'nested': False,
            'arn_format': 'arn:aws:route53profiles:{region}:{account_id}:profile-association/{resource_id}'
        }
    }
...
def discovery(self, session, account_id, region, service, service_type, logger):    
    ...
    try:
        ...
        # Route53 Profiles is regional
        client = session.client('route53profiles', region_name=region)
        
        if not hasattr(client, config['method']):
            raise ValueError(f"Method {config['method']} not available for route53profiles client")
...
def tagging(account_id, region, service, client, resources, tags_string, tags_action, logger):
    ...
    # Create Route53 Profiles client
    session = boto3.Session()
    profiles_client = session.client('route53profiles', region_name=region)

    for resource in resources:            
        try:
            if tags_action == 1:
                # Add tags
                profiles_client.tag_resource(
                    ResourceArn=resource.arn,
                    Tags=tags
                )
            elif tags_action == 2:
                # Remove tags
                profiles_client.untag_resource(
                    ResourceArn=resource.arn,
                    TagKeys=tag_keys
                )
