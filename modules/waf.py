...
def get_service_types(account_id, region, service, service_type):
    """
    AWS WAF resources that support tagging.
    Based on: https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/waf/client/tag_resource.html
    
    WAF supports tagging for:
    - WebACL (Web Access Control Lists)
    - Rule (WAF Rules)
    - RuleGroup (Rule Groups)
    - ByteMatchSet (Byte match sets)
    - IPSet (IP sets)
    - GeoMatchSet (Geographic match sets)
    - RegexMatchSet (Regular expression match sets)
    - RegexPatternSet (Regular expression pattern sets)
    - SizeConstraintSet (Size constraint sets)
    - SqlInjectionMatchSet (SQL injection match sets)
    - XssMatchSet (Cross-site scripting match sets)
    - RateBasedRule (Rate-based rules)
    
    Note: WAF Classic is a global service but resources are region-specific
    """
    ...
    resource_configs = {
        'WebACL': {
            ...
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:webacl/{resource_id}'
        },
        'Rule': {
            'method': 'list_rules',
            'key': 'Rules',
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:rule/{resource_id}'
        },
        'RuleGroup': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:rulegroup/{resource_id}'
        },
        'ByteMatchSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:bytematchset/{resource_id}'
        },
        'IPSet': {
            'method': 'list_ip_sets',
            'key': 'IPSets',
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:ipset/{resource_id}'
        },
        'GeoMatchSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:geomatchset/{resource_id}'
        },
        'RegexMatchSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:regexmatchset/{resource_id}'
        },
        'RegexPatternSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:regexpatternset/{resource_id}'
        },
        'SizeConstraintSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:sizeconstraintset/{resource_id}'
        },
        'SqlInjectionMatchSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:sqlinjectionmatchset/{resource_id}'
        },
        'XssMatchSet': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:xssmatchset/{resource_id}'
        },
        'RateBasedRule': {
            ...
            'date_field': None,  # Not available in list response
            'nested': False,
            'arn_format': 'arn:aws:waf::{account_id}:ratebasedrule/{resource_id}'
        }
    }
...
def discovery(self, session, account_id, region, service, service_type, logger):    
    ...
    try:
        ...
        # WAF Classic is a global service but accessed regionally
        client = session.client('waf', region_name=region)
        
        if not hasattr(client, config['method']):
            raise ValueError(f"Method {config['method']} not available for waf client")
        ...
        for page in page_iterator:
            ...
            for item in items:
                ...
                # Get creation date (not available in WAF list responses)
                creation_date = None

                # Build ARN - WAF uses specific ARN format
                ...
                try:
                    ...
                    # Convert WAF tag format to standard format
...
def tagging(account_id, region, service, client, resources, tags_string, tags_action, logger):
    ...
    waf_client = session.client('waf', region_name=region)

    for resource in resources:            
        try:
            if tags_action == 1:
                # Add tags - Convert to WAF format (list of objects)
                waf_tags = [{'Key': tag['Key'], 'Value': tag['Value']} for tag in tags]
                waf_client.tag_resource(
