...
def get_service_types(account_id, region, service, service_type):
    resource_configs = {
        'Repository': {
            ...
            'nested': False,
            'arn_format': 'arn:aws:ecr:{region}:{account_id}:repository/{resource_id}'
        }
    }
...
def discovery(self, session, account_id, region, service, service_type, logger):
    ...
    try:
        ...
        for page in response_iterator:
            if config['nested']:
                items = []  # ECR doesn't have nested resources in the response
            else:
            ...
            for item in items:
                ...
                try:
                    tag_response = client.list_tags_for_resource(resourceArn=item['repositoryArn'])
                    resource_tags = {tag['Key']: tag['Value'] for tag in tag_response.get('tags', [])}
                except Exception as e:
                    logger.warning(f"Error fetching tags for repository {resource_id}: {str(e)}")
                    resource_tags = {}
...
def tagging(account_id, region, service, client, resources, tags_string, tags_action, logger):
    ...
    for resource in resources:
        try:
            if tags_action == 1:  # Create tags
                client.tag_resource(
                    resourceArn=resource.arn,
                    tags=tags
                )
            elif tags_action == 2:  # Delete tags
