...
aws cloudformation create-stack --stack-name "$stack_name-frontend-$id" --parameters ParameterKey=Username,ParameterValue=snmatus@amazon.com ParameterKey=S3Artifacts,ParameterValue=$stack_name-$id ParameterKey=DSQLCluster,ParameterValue=$dsql_cluster_endpoint ParameterKey=DSQLClusterId,ParameterValue=$dsql_cluster_identifier --template-body file://$template_frontend --region us-east-1 --capabilities CAPABILITY_NAMED_IAM
