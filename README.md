...
2.	Navigate to the AWS CloudFormation service
    o	From the AWS Console, search for "CloudFormation" in the search bar
    o	Click on the CloudFormation service
3.	Start Stack Creation
    o	Click the "Create stack" button
    o	Select "With new resources (standard)"
4.	Specify Template Source
    o	Choose "Upload a template file" 
    o	Click "Choose file" and select your CloudFormation template (cloudformation.public.yaml)    
    o	Click "Next"
5.	Specify Stack Details
    o	Enter a meaningful name for the stack in the "Stack name" field (e.g., "tagger-solution-frontend")
6.	Configure General Configuration Parameters
    o	GitHubRepositoryUrl: Enter the HTTPS URL for your GitHub repository (e.g., https://github.com/aws-samples/sample-tagger.git)
    o	AppUser: Enter the email address that will be used for the application user (e.g., admin@example.com )
7.	Configure Network and Security Parameters
    o	WAFRequired: Select "true" if you want to enable AWS WAF protection, or "false" to disable it
    o	IPv4CIDR: (Optional, required only if WAF is enabled) Enter the IPv4 CIDR range that should be allowed access (e.g., 192.168.1.0/24 or 0.0.0.0/0 for all IPv4 addresses)
    o	IPv6CIDR: (Optional, required only if WAF is enabled) Enter the IPv6 CIDR range that should be allowed access (e.g., 2001:db8::/32 or leave empty if not needed)
8.	Configure Stack Options (Optional)
    o	Add any tags to help identify and manage your stack resources
    o	Configure any advanced options like stack policy, rollback configuration, etc.
    o	Click "Next"
9.	Review Stack Configuration
    o	Review all the parameters and settings for your stack
    o	Scroll down and check the acknowledgment box that states "I acknowledge that AWS CloudFormation might create IAM resources with custom names"
    o	Click "Create stack"
10.	Monitor Stack Creation
    o	The CloudFormation console will display the stack creation status
    o	You can view the "Events" tab to monitor the progress and troubleshoot any issues
    o	Wait until the stack status changes to "CREATE_COMPLETE"
11.	Access Stack Outputs
    o	Once the stack creation is complete, navigate to the "Outputs" tab
    o	Here you'll find important information such as the URL to access the frontend application
12.	Verify Deployment
    o	Navigate to the provided URL to ensure the frontend is working correctly
    o	Log in using the provided application user email and credentials (you may receive temporary credentials via email)


### Private method access version

1.	Sign in to the AWS Management Console
    o	Navigate to the AWS Console at https://console.aws.amazon.com/ 
    o	Sign in with your AWS account credentials
2.	Navigate to the AWS CloudFormation service
    o	From the AWS Console, search for "CloudFormation" in the search bar at the top
    o	Click on the CloudFormation service from the dropdown results
3.	Start Stack Creation
    o	Click the "Create stack" button
    o	Select "With new resources (standard)" from the dropdown options
4.	Specify Template Source
    o	Choose "Upload a template file" 
    o	Click "Choose file" and select your CloudFormation template (cloudformation.private.yaml)    
    o	Click "Next"
5.	Specify Stack Details
    o	Enter a meaningful name for the stack in the "Stack name" field (e.g., "tagger-solution-frontend")
6.	Configure General Configuration Parameters
    ...
    o	AppUser: Enter the email address that will be used for the application user (e.g., admin@example.com )
7.	Configure Network and Security Parameters
    o	VPCId: Select the VPC ID from where the App Runner service will be accessed
    o	SubnetId: Select the Subnet ID from where the App Runner service will be accessed
    o	IPv4CIDR: Enter the IPv4 CIDR range that should be allowed access the App Runner service (e.g., 192.168.1.0/24 or 0.0.0.0/0 for all IPv4 addresses)
    o	IPv6CIDR: Enter the IPv6 CIDR range that should be allowed access the App Runner service (e.g., 2001:db8::/32 or ::/0 for all IPv6 addresses)
8.	Configure Stack Options (Optional)
    o	Add any tags to help identify and manage your stack resources
    o	Configure any advanced options like stack policy, rollback configuration, etc.
    o	Click "Next"
...
11.	Access Stack Outputs
    o	Once the stack creation is complete, navigate to the "Outputs" tab
    o	Here you'll find important information such as the URL to access the frontend application
    o	Note down the App Runner service URL or any other important outputs
12.	Verify Deployment
    o	Navigate to the provided URL to ensure the frontend is working correctly
    o	Log in using the provided application user email (you may receive temporary credentials via email)
